...
public class HtmlProxyAjaxModule extends lebah.portal.AjaxBasedModule implements lebah.portal.HtmlContainer {
	...
	private static Hashtable<String, String> contentData = new Hashtable<String, String>();
	...
	public String doTemplate2() throws Exception {
		...
		if ( !firstTime ) {
			...
			String func = "loadURL" + formName + "('" + strUrl + "')";
			String str = "<a href=\"javascript:" + func + "\"><img src=\"../img/indicator.gif\" border=\"0\"></a>";
			str += "<script>self.setTimeout(\"" + func + "\", 1200)</script>";
			//context.put("stringbuffer", str);
			contentData.put(moduleId, str);
	...
	private void showPage(HttpSession session) {
		try {
			renderPage(session);
			context.put("content", contentData);
		} catch ( Exception e ) {
	...
	private void renderPage(HttpSession session) throws Exception {
		...
		try {
	    	...
	    	*/
			...
			//remove <style>
			buf = replacing(action, url2, buf);
			contentData.put(moduleId, buf.toString());
			//context.put("stringbuffer", buf);
			//context.put("content", contentCacher);
	...
	private StringBuffer replacing(String action, String url2, StringBuffer buf) {
	...
	void renderFirstPage(String str) {
	...
	private void createFirstPage(String url) throws Exception {
		
		String first = "<a href=\"" + url + "\">";
