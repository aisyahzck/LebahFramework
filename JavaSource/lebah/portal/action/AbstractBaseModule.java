 ...
 * #	Date			Name				Remarks
 * ----	--------------	------------------	---------------------------------------------------
 * 1.	Aug 11, 2009	taufek				File created.
 * 2. 	Aug 20, 2009	taufek				Added method to add message with parameters.
 * 3. 	Aug 20, 2009	taufek				Added pid to session key.
 */
...
public abstract class AbstractBaseModule extends AjaxModule {
	...
	private void preProcess() throws Exception {
		...
		/*
		 * Process fields with @HttpRequestParameter or @HttpSessionAttribute
		 */
		if (fields != null) {
			for (Field field : fields) {

				HttpRequestParameter httpReqAnno = field
						.getAnnotation(HttpRequestParameter.class);

				HttpSessionAttribute httpSesAnno = field
						.getAnnotation(HttpSessionAttribute.class);
				...
				if (httpReqAnno != null) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("setting field[" + field + "] with value["
								+ getParam(paramKey) + "] from HttpRequest.");
					}
				...
				if (httpSesAnno != null) {
					field.setAccessible(true);
					Object obj = getHttpSession().getAttribute(getId() + "_" + 
							httpSesAnno.attributeKey());

					if (logger.isDebugEnabled()) {
						logger.debug("setting field[" + field + "] with value["
								+ obj + "] from HttpSession.");
					}
					field.set(this, obj);
				}
			}
	...
	private void processMethodToInvoke() throws Exception {
		...
		for (Method mtd : methods) {
			...
			MappedCommand mappedCommand = mtd
					.getAnnotation(MappedCommand.class);

			if (mappedCommand != null
					&& mappedCommand.command().equals(command)) {
	...
	private void postProcess() throws Exception {
		...
		/*
		 * Populating HttpSession with objects annotated with
		 * 
		...
		if (fields != null) {
			for (Field field : fields) {
				...
				VelocityContextParameter velCtxAnno = field
						.getAnnotation(VelocityContextParameter.class);

				if (httpSesAnno != null) {
					field.setAccessible(true);
					if (logger.isDebugEnabled()) {
						logger.debug("setting HttpSession with: "
								+ httpSesAnno.attributeKey() + ", "
								+ field.get(this));
					}

					getHttpSession().setAttribute(getId() + "_" + httpSesAnno.attributeKey(),
							field.get(this));
				}
				...
				if (velCtxAnno != null) {
					field.setAccessible(true);
					if (logger.isDebugEnabled()) {
						logger.debug("setting context with: "
								+ velCtxAnno.paramName() + ", "
								+ field.get(this));
					}
